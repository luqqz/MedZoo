    ...
    for dep in range(tile_deps):
        for row in range(tile_rows):
            for col in range(tile_cols):
                abc += 1
                print("Prediction " + str(abc) + "/" + str(tile_deps * tile_rows * tile_cols))
                d1 = int(dep * strideD)
                x1 = int(col * strideHW)
                y1 = int(row * strideHW)
...
def save_nii(args, pred, label, name, affine, task_id): # bs, c, WHD
    ...
    for tt in range(seg_pred.shape[0]):
        ...
        seg_label_save_p = os.path.join(args.save_path + '/%s_label_task%d.nii.gz' % (name[tt], task_id))
        seg_pred_save_p = os.path.join(args.save_path + '/%s_pred_task%d.nii.gz' % (name[tt], task_id))
...
def validate(args, input_size, model, ValLoader, num_classes, engine):
    ...
    for index, batch in enumerate(ValLoader):
        ...
        with torch.no_grad():
            ...
            # save
            save_nii(args, pred_sigmoid, label, name, affine, task_id)
