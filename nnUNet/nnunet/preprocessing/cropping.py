...
#     mask_voxel_coords = np.where(mask[0] != outside_value)
#     minzidx = int(np.min(mask_voxel_coords[0])) - 5
#     if minzidx < 0:
#         minzidx = 0
#     maxzidx = int(np.max(mask_voxel_coords[0])) + 5
#     minxidx = int(np.min(mask_voxel_coords[1])) - 50
#     if minxidx < 0:
#         minxidx = 0
#     maxxidx = int(np.max(mask_voxel_coords[1])) + 50
#     diff = maxxidx - minxidx
#     minyidx = int(np.min(mask_voxel_coords[2])) - 50
#     if minyidx < 0:
#         minyidx = 0
#     maxyidx = minyidx + diff
