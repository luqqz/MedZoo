...
if __name__ == "__main__":
    ...
    checkpoing_dir = "/content/drive/MyDrive/MGR/PIPO-FAN/chk"
    if not path.isdir(checkpoing_dir):
        os.makedirs(checkpoing_dir)
        
    log_dir = "/content/drive/MyDrive/MGR/PIPO-FAN/chk"
    ...
    resunet_checkpoint = torch.load('/content/drive/MyDrive/MGR/PIPO-FAN/chk/resu_checkpoint_ep1425.pth.tar')
    resunet_dict = resunet_checkpoint['state_dict']

    # from collections import OrderedDict
    # new_state_dict = OrderedDict()
    # for k, v in resunet_dict.items():
    #     name = k#[7:] # remove `resnet.`
    #     if 'resnet' in name:
    ...
    folder_training_1 = '/content/drive/MyDrive/MGR/PIPOFAN_DATA/prect/lits/'
    folder_validation_1 = '/content/drive/MyDrive/MGR/PIPOFAN_DATA/prect/lits_val/'

    folder_training_2 = '/content/drive/MyDrive/MGR/PIPOFAN_DATA/prect/kits/'
    #folder_validation_2 = '/content/drive/MyDrive/MGR/PIPOFAN_DATA/prect/kits_val/'

    folder_training_3 = '/content/drive/MyDrive/MGR/PIPOFAN_DATA/prect/spleen/'
    #folder_validation_3 = '/content/drive/MyDrive/MGR/PIPOFAN_DATA/prect/spleen_val/'
...
#Dataset4
    ...
    print(resunet_checkpoint['epoch'])
    ep = resunet_checkpoint['epoch']
    for epoch in range(ep, num_epochs):
        ...
        val_loss = validate(val_loader1, '4', model, criterion, epoch, verbose=True)
        val_history.append(val_loss)
        # val_loss = validate(val_loader2, '4', model, criterion, epoch, verbose=True)
        # val_history.append(val_loss)
        # val_loss = validate(val_loader3, '4', model, criterion, epoch, verbose=True)
        # val_history.append(val_loss)
        ...
        if epoch % 10 == 0:
            filename = path.join(checkpoing_dir, 'resunet_checkpoint_ep{:04d}.pth.tar'.format(epoch + 1))
            torch.save({'epoch': epoch + 1,
                             'state_dict': model.state_dict(),
                             'best_dice': best_dice,
                             'optimizer' : optimizer.state_dict(),},filename)
